/* BodyFit - Single-file React app
   Notes:
   - This file is the React component only. Do NOT include the surrounding <!DOCTYPE html> ... </html>
     when you paste into src/App.jsx of a create-react-app project.
   - The original error most users hit is "Unexpected token '<'" or a JSX/parse error because the full
     HTML document was pasted into a .jsx file. The fix is to keep only the component source below.
*/
...
export default function App() {
  ...
  // ensure pose is initialized once both pose lib is loaded and the video element is ready/playing
  useEffect(() => {
    if (poseLoaded && videoRef.current && (videoRef.current.readyState >= 2 || !videoRef.current.paused)) {
      initPose();
    }
  ...
  async function startCamera() {
    ...
    try {
      ...
      // playsInline attribute in markup already present; ensure playback
      await videoRef.current.play().catch(() => { /* autoplay may be blocked until user interacts; ignore */ });
      setStatus('camera started');
      // If pose lib already loaded, init; otherwise the useEffect watching poseLoaded will call initPose once loaded.
      if (poseLoaded) initPose();
    } catch (e) {
      console.error(e);
  ...
  function initPose() {
    ...
    if (window.Camera && videoRef.current) {
      ...
      // attach cleanup to poseRef so if component unmounts we clear it
      poseRef.current._cleanupInterval = () => clearInterval(interval);
    }
    setStatus('pose initialized');
  }
  ...
  function onResults(results) {
    ...
    canvas.width = videoRef.current.videoWidth || videoRef.current.clientWidth || 640;
    canvas.height = videoRef.current.videoHeight || videoRef.current.clientHeight || 480;
    ...
    if (results.poseLandmarks) {
      ...
      if (scaleCmPerPx || heightCm) {
        ...
        if (skin) {
          setSkinRgb(skin.rgb);
          const lab = rgbToLab(skin.rgb);
          setSkinLab(lab);
          const recs = recommendOutfits(meas, lab);
  ...
  function computeMeasurements(lm, w, h, scale, knownHeight) {
    ...
    const shoulderPx = (leftShoulder && rightShoulder) ? pxDistance(leftShoulder, rightShoulder, w, h) : null;
    const hipPx = (leftHip && rightHip) ? pxDistance(leftHip, rightHip, w, h) : null;
    const headToAnklePx = (topHead && leftAnkle) ? pxDistance(topHead, leftAnkle, w, h) : null;

    // determine scale
    let S = scale;
    if (!S && knownHeight && headToAnklePx) {
      S = parseFloat(knownHeight) / headToAnklePx; // cm per px
      // save scale for further frames
      setScaleCmPerPx(S);
    }

    const shoulderCm = S && shoulderPx ? shoulderPx * S : null;
    const hipCm = S && hipPx ? hipPx * S : null;
